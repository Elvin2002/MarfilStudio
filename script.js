...
try {
	...
	if (!_xnext_included && !window.location.href.match(/fb_xd_fragment/g)) {
		...
		window.ec.events = window.ecwidEvents; (function () {
			...
			tryInjectRosettaScript('https://d3cy3u1txmkqs3.cloudfront.net/rosetta/translations2022/new-frontend/files/new-frontend.es_MX.-462718174.js', 'https://ecwid-rosetta.s3.amazonaws.com/translations2022/new-frontend/files/new-frontend.es_MX.-462718174.js');
		})();
		(function () {
			window.ecwid_init_elm_config = function () {
				function load() {
					...
					script.src = selectJsVersion('https://d3cy3u1txmkqs3.cloudfront.net/venera/static/ecwid-storefront.00eeed5f1d584bc9ef2f3e6c0fb30a05.min.js');
					document.getElementsByTagName('head')[0].appendChild(script);
				}
				function loadCheckout(callback) {
					...
					script.src = selectJsVersion('https://d3cy3u1txmkqs3.cloudfront.net/venera/static/ecwid-checkout.dc1e97c8f8725f5b1b513432d02500cf.min.js');
					document.getElementsByTagName('head')[0].appendChild(script);
		...
		EcwidObj.storefrontVersion = "2023-57295-g47de43a9c8f73b";
		EcwidObj.MessageBundles = EcwidObj.MessageBundles ? EcwidObj.MessageBundles : {};
		EcwidObj.restoreCartData = EcwidObj.restoreCartData ? EcwidObj.restoreCartData : {};
		EcwidObj.ExtensionPoint = ep;
		EcwidObj.ProductBrowser = {
			Links: new ep,
			CategoryView: proxyChain()
		};
		EcwidObj.Controller = proxyChain();
		EcwidObj.ShoppingCartController = proxyChain();
		EcwidObj.ProductModel = proxyChain();
		EcwidObj.CategoriesModel = proxyChain();
		EcwidObj.CategoryModel = proxyChain();
		EcwidObj.AppContainer = proxyChain();
		EcwidObj.Profile = proxyChain();
		EcwidObj.CustomerCredentialsModel = proxyChain();
		EcwidObj.LocationHashModel = proxyChain();
		EcwidObj.OnAPILoaded = new ep;
		EcwidObj.OnPageLoad = new ep;
		EcwidObj.OnSetProfile = new ep;
		EcwidObj.OnPageLoaded = new ep;
		EcwidObj.OnConfigLoaded = new ep;
		EcwidObj.OnCartChanged = new ep;
		EcwidObj.OnOrderPlaced = new ep;
		EcwidObj.OnProductOptionsChanged = new ep;
		EcwidObj.OnConsentChanged = new ep;
		EcwidObj.OnPageSwitch = new ep;
		EcwidObj.signature = "a88e6c3b452bb89b36e370de907068f8-0";
		EcwidObj._isInteractiveEnabled = (function () {
			var cache = null;
		...
		EcwidObj._injectEcwidCss = function () {
			...
			window.ec_legacyCssUrl = 'https://d1oxsl77a1kjht.cloudfront.net/css/new?hc=1765459971&ownerid=14019364&pswpNative=true' + cssUrlAddition;
			var cssUrl = 'https://d1oxsl77a1kjht.cloudfront.net/css/new?hc=1765459971&ownerid=14019364&pswpNative=true' + cssUrlAddition;
...
<div class="ecwid-pb-placeholder__product"></div><div class="ecwid-pb-placeholder__title"><div></div><div></div><div></div></div>\
		...
		EcwidObj._onBodyDone = function () {
			if ((!window.ecwid_bodyDone && !EcwidObj._hasFacebookIframe() && !window.ecwid_dynamic_widgets) || window.ecwid_dynamic_widgets && !window.ecwid_bodyDone) {
				...
				ru_cdev_xnext_frontend_LegacyFrontend = function () {
					...
					function C(f) {
						function g(a) {
							function b() { if (typeof q.readyState == jb) { return typeof q.body != jb && q.body != null } return /loaded|complete/.test(q.readyState) }
					...
					function D(e) {
						function f(a) { var b = w(); var c = b.body; var d = b.createElement(mb); d.language = ob; d.src = a; d.async = true; if (ru_cdev_xnext_frontend_LegacyFrontend.__errFn) { d.onerror = function () { ru_cdev_xnext_frontend_LegacyFrontend.__errFn(pb, new Error(qb + a)) } } c.appendChild(d); s(rb, sb) }
						s(rb, tb); f(e)
					}
					...
					function G() {
						if (window.ecwid_script_base) { o = window.ecwid_script_base; return o } function e(a) { var b = a.lastIndexOf(Gb); if (b == -1) { b = a.length } var c = a.indexOf(Hb); if (c == -1) { c = a.length } var d = a.lastIndexOf(Ib, Math.min(c, b)); return d >= _ ? a.substring(_, d + ab) : hb }
					...
					function I() {
						var f = []; var g = _; function h(a, b) { var c = f; for (var d = _, e = a.length - ab; d < e; ++d) { c = c[a[d]] || (c[a[d]] = []) } c[a[e]] = b }
						var j = []; var k = []; function l(a) { var b = k[a](), c = j[a]; if (b in c) { return b } var d = []; for (var e in c) { d[c[e]] = e } if (t) { t(a, d, b) } throw null }
						k[Pb] = function () { var a = navigator.userAgent.toLowerCase(); var b = q.documentMode; if (function () { return a.indexOf(Qb) != -1 }()) return Rb; if (function () { return a.indexOf(Sb) != -1 && (b >= lb && b < Tb) || a.indexOf(Ub) != -1 }()) return Vb; if (function () { return a.indexOf(Sb) != -1 && (b >= Wb && b < Tb) }()) return Xb; if (function () { return a.indexOf(Yb) != -1 || (a.indexOf(Zb) != -1 || a.indexOf(Sb) != -1 && (b >= $b && b < Tb)) }()) return _b; if (function () { return a.indexOf(ac) != -1 || (b >= Tb || a.indexOf(bc) != -1) }()) return cc; return Rb }; j[Pb] = { 'gecko1_8': _, 'ie10': ab, 'ie8': dc, 'ie9': ec, 'safari': fc }; __gwt_isKnownPropertyValue = function (a, b) { return b in j[a] }; ru_cdev_xnext_frontend_LegacyFrontend.__getPropMap = function () { var a = {}; for (var b in j) { if (j.hasOwnProperty(b)) { a[b] = l(b) } } return a }; ru_cdev_xnext_frontend_LegacyFrontend.__computePropValue = l; p.__gwt_activeModules[Y].bindings = ru_cdev_xnext_frontend_LegacyFrontend.__getPropMap; s(U, gc); if (r()) { return H(hc) } var m; try { h([cc], ic); h([_b], jc); h([Rb], kc); h([Xb], lc); h([Vb], mc); m = f[l(Pb)]; var n = m.indexOf(nc); if (n != -1) { g = parseInt(m.substring(n + ab), lb); m = m.substring(_, n) } } catch (a) { } ru_cdev_xnext_frontend_LegacyFrontend.__softPermutationId = g; return H(m + oc)
					}
					function J() { if (!p.__gwt_stylesLoaded) { p.__gwt_stylesLoaded = {} } s(pc, V); s(pc, qc) }
					F(); ru_cdev_xnext_frontend_LegacyFrontend.__moduleBase = G(); u[Y].moduleBase = ru_cdev_xnext_frontend_LegacyFrontend.__moduleBase; var K = I(); if (p) {
						var L = !!(p.location.protocol == rc || (p.location.protocol == sc || p.location.protocol == tc)); p.__gwt_activeModules[Y].canRedirect = L; function M() { var b = uc; try { p.sessionStorage.setItem(b, b); p.sessionStorage.removeItem(b); return true } catch (a) { return false } }
						if (L && M()) { var N = vc; var O = p.sessionStorage[N]; if (!/^https:\/\/\w+.local.ecwid.com(:\d+)?\/.*$/.test(O)) { if (O && (window.console && console.log)) { console.log(wc + O) } O = hb } if (O && !p[N]) { p[N] = true; p[N + xc] = G(); var P = q.createElement(mb); P.src = O; var Q = q.getElementsByTagName(yc)[_]; Q.insertBefore(P, Q.firstElementChild || Q.children[_]); return false } }
					} J(); s(U, qc); function R(b) { var c = zc; var d = setInterval(function () { if (!c) { clearInterval(d) } else { var a = document.getElementById(ru_cdev_xnext_frontend_LegacyFrontend.__moduleName).contentWindow.document.getElementsByTagName(mb); if (a.length !== _ && a[_].hasAttribute(Ac)) { clearInterval(d); b() } c-- } }, Bc) }
				...
				// ========================= STUBBED NOCACHE END ===========================
				if (!EcwidObj._isInteractiveEnabled()) {
					EcwidObj._legacyAfterBodyDone();
				}
			}
		};
		EcwidObj._onComplete = function () {
		};
		EcwidObj._legacyAfterBodyDone = function () {
			window.ecwid_script_defer = true;
			if (document.removeEventListener) {
				document.removeEventListener("DOMContentLoaded", EcwidObj._onBodyDone, false);
			}
			if (window.ecwid_onBodyDoneTimerId) {
				clearInterval(window.ecwid_onBodyDoneTimerId);
			}
		};
		EcwidObj._isIos = function () {
			return navigator.userAgent.match(/(iPod|iPhone|iPad)/) != null;
		};
		EcwidObj._fixIosIframeBlur = function () {
		...
		EcwidObj._getDisabledAppsSetting = (function () {
		...
		EcwidObj._isAppEnabled = function (namespace) {
			var disabledAppSetting = EcwidObj._getDisabledAppsSetting();
		...
		EcwidObj._filter = function (arr, fn) {
			var result = [];
		...
		EcwidObj._forEach = function (arr, fn) {
			for (var i = 0, len = arr.length; i < len; i++) {
				fn(arr[i]);
			}
		};
		...
		if (EcwidObj._isIos()) {
			EcwidObj._fixIosIframeBlur();
		}
		window.ecwid_onBodyDoneTimerId = null;
		window.ecwid_bodyDone = false;
		window.ecwid_onBodyDone = EcwidObj._onBodyDone;
		window.needLoadEcwidAsync = EcwidObj._isInteractiveEnabled(); //need for async loading in gwt library
		window.allowGWTAppResurrection = false;
		xInjectJs('https://d3dq8sxcny4hg.cloudfront.net/states.js?version=2023-57295-g47de43a9c8f73b&callback=window.ecwid_states_data.loaded');
		...
		window.ecwid_script_base = "https://d11s7fcxy18ubx.cloudfront.net/node/static/2023/2023-57295-g47de43a9c8f73b/ru.cdev.xnext.frontend.LegacyFrontend/";
		window.ecwid_static_base = "https://d11s7fcxy18ubx.cloudfront.net/node/static/2023/2023-57295-g47de43a9c8f73b/"
		...
		window.ecwid_loader_logo_url = "https://d1dkdnyvras0l5.cloudfront.net/dudamobile/loading.gif";
		if (!window.amazon_image_domain) {
			window.amazon_image_domain = "https://d2j6dbq0eux0bg.cloudfront.net";
		}
		window.xnext_ownerId = 14019364;
		EcwidObj.publicToken = "public_QAEZEnEhzPxz9F9FJ5RfL4L2nnWv9spY",
			EcwidObj.demo = false;
		EcwidObj.cssUrl = "https://d1oxsl77a1kjht.cloudfront.net/css/new?hc=1765459971&ownerid=14019364&pswpNative=true";
		EcwidObj.acceptLanguage = ["es_419", "es"];
		EcwidObj.appDomain = "https://app.multiscreenstore.com/";
		EcwidObj.startup_flags = {
		...
		(function () {
			...
			var ecDataJsBaseUrl = "https://app.multiscreenstore.com/data.js?ownerid=14019364";
			ecDataJsBaseUrl += "&lang=en";
			ecDataJsBaseUrl += "&token=3e4e43d91c37ae855d24442131058a716909939a";
		...
		EcwidObj.getAppPublicToken = function (namespace) {
			var tokenMap = { "ecwid-storefront": "public_QAEZEnEhzPxz9F9FJ5RfL4L2nnWv9spY" };
			var result = tokenMap[namespace];
			return typeof result === 'string' ? result : null;
			return null
		}
		if (EcwidObj._isInteractiveEnabled()) {
			EcwidObj._loadEcwidAsync();
		} else {
			EcwidObj._loadEcwidSync();
		}
		if (!window.__heap_inits__ && !(((window.ec || {}).config || {}).disable_all_cookies === true)) {
			(function (i, s, o, g, r, a, m) {
				i['HeapAnalyticsObject'] = r; i[r] = i[r] || function () { (i[r].q = i[r].q || []).push(arguments) }, i[r].l = 1 * new Date(); a = s.createElement(o), m = s.getElementsByTagName(o)[0];
				a.async = 1; a.src = g; if (EcwidObj._isInteractiveEnabled() && m != null && m.parentNode != null) m.parentNode.insertBefore(a, m); else var h = 100, k = setInterval(function () { m = s.getElementsByTagName(o)[0]; if ((/loaded|complete/.test(s.readyState) || 0 == h--) && m != null && m.parentNode != null) m.parentNode.insertBefore(a, m), clearInterval(k) }, 100)
			})(window, document, "script", "https://ecomm.events/i.js", "eca");
			window.__heap_inits__ = true
		}
		window.parseId = window.parseId || function (args) {
			var idPrefix = "id=";
			var id;
		...
		window.parseStyle = window.parseStyle || function (args) {
		...
		window.xAddWidget = window.xAddWidget || function (widgetType, args) {
...
EcwidObj.getAppPublicConfig = function (namespace) {
	var publicData = { "dudamobile": "{}", "ecwid-storefront": "{}" };
